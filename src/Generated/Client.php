...
class Client extends \Datenkraft\Backbone\Client\FulfillmentApi\Generated\Runtime\Client\Client
{
    ...
    *     @var string $filter[shopCode] The shopCode used internally to distinguish between clients. \
    _This code is optional, if your identity is assigned to only one shop.
    Otherwise the response would be a 422 HTTP Error._
    *     @var string $filter[packagingCode] Filter for an exact packagingCode.
    * }
    * @param string $fetch Fetch mode to use (can be OBJECT or RESPONSE)
    ...
    *     @var string $shopCode The shopCode used internally to distinguish between clients. \
    _This code is optional, if your identity is assigned to only one shop.
    Otherwise the response would be a 422 HTTP Error._
    *     @var string $filter[status] Only return stock checks with one or more of the given statuses (comma separated):
    - `open`: not yet processed by the warehouse
    - `finished`: processed by the warehouse
    - `nullified`: cancelled without processing
    *     @var string $filter[type] Only return stock checks of one or more of the given types (comma separated):
